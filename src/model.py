...
class PIHAM(nn.Module):
    ...
    def fit(
        ...
        for s in np.random.choice(np.arange(100), size=N_seeds, replace=False):
            ...
            optimizer = optim.Adam(
                [self.U, self.V, self.W, self.Hcategorical, self.Hpoisson, self.Hgaussian],
                lr=learning_rate,
            )
        ...
        self.Hpoisson = nn.Parameter(best_Hpoisson.clone().detach(), requires_grad=True).to(
            self.device
        )
        self.Hgaussian = nn.Parameter(best_Hgaussian.clone().detach(), requires_grad=True).to(
            self.device
        )
    ...
    # compute positive log posterior
    def log_posterior(
            self,
            U: Tensor,
            V: Tensor,
    ...
    # define UVW as one parameter to compute the joint Hessian
